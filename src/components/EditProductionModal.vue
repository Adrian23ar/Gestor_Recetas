...
const formattedNetProfit = computed({
    get() {
        ...
        // Devuelve string vacío si es null/undefined o NaN, sino formateado
        return editableRecord.value?.netProfit === null || isNaN(num) ? '' : num.toFixed(2);
    },
    set(value) {
        if (editableRecord.value) {
            const parsed = parseFloat(value);
            // Guarda null si está vacío o no es número, sino el número parseado
                ...
                <form @submit.prevent="saveChanges" class="space-y-4">
                    ...
                    <div>
                        <label for="edit-net-profit"
                            class="block text-sm font-medium text-text-base dark:text-dark-text-base">Ganancia Neta ($):</label>
